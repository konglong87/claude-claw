...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  console.log('[managedEnv-safe] 🔍 应用安全环境变量 (初始化前)')
  Object.assign(process.env, filterSettingsEnv(getGlobalConfig().env))

  // Apply config.yaml env early (trusted project-level config)
  // This gives config.yaml higher priority than settings.json
  try {
    const { getConfig } = require('../config/loader.js') as typeof import('../config/loader.js')
    const configEnv = getConfig().env
    if (configEnv && typeof configEnv === 'object' && Object.keys(configEnv).length > 0) {
      console.log('[managedEnv-safe] 🔍 应用 config.yaml env (可信源):', Object.keys(configEnv))
      console.log('[managedEnv-safe] 🔍 config.yaml env 详情:', JSON.stringify(configEnv, null, 2))
      Object.assign(process.env, filterSettingsEnv(configEnv))
    } else {
      console.log('[managedEnv-safe] 🔍 config.yaml 无 env 或为空')
    }
  } catch (error) {
    // config.yaml may not exist, ignore
    console.log('[managedEnv-safe] 🔍 加载 config.yaml 失败:', error)
  }
 ...
 * 1. Global config (~/.claude.json) - lowest
 * 2. Settings.json env
 * 3. config.yaml env - highest (project-level config)
 */
export function applyConfigEnvironmentVariables(): void {
  console.log('[managedEnv] 🔍 开始应用环境变量...')

  // 1. Apply global config env (lowest priority)
  const globalEnv = getGlobalConfig().env
  if (globalEnv && Object.keys(globalEnv).length > 0) {
    console.log('[managedEnv] 🔍 Step 1: 应用 ~/.claude.json env:', Object.keys(globalEnv))
    Object.assign(process.env, filterSettingsEnv(globalEnv))
  } else {
    console.log('[managedEnv] 🔍 Step 1: ~/.claude.json 无 env 配置')
  }

  // 2. Apply settings.json env (medium priority)
  const settingsEnv = getSettings_DEPRECATED()?.env
  if (settingsEnv && Object.keys(settingsEnv).length > 0) {
    console.log('[managedEnv] 🔍 Step 2: 应用 settings.json env:', Object.keys(settingsEnv))
    console.log('[managedEnv] 🔍 settings.json env 详情:', JSON.stringify(settingsEnv, null, 2))
    Object.assign(process.env, filterSettingsEnv(settingsEnv))
  } else {
    console.log('[managedEnv] 🔍 Step 2: settings.json 无 env 配置')
  }

  // 3. Apply config.yaml env (highest priority)
  // This ensures project-level config.yaml overrides user-level settings.json
  try {
    const { getConfig } = require('../config/loader.js') as typeof import('../config/loader.js')
    const configEnv = getConfig().env
    if (configEnv && typeof configEnv === 'object' && Object.keys(configEnv).length > 0) {
      console.log('[managedEnv] 🔍 Step 3: 应用 config.yaml env (最高优先级):', Object.keys(configEnv))
      console.log('[managedEnv] 🔍 config.yaml env 详情:', JSON.stringify(configEnv, null, 2))
      Object.assign(process.env, filterSettingsEnv(configEnv))

      // 🔍 验证优先级：打印最终的 process.env 值
      console.log('[managedEnv] 🔍 验证优先级结果:')
      for (const key of Object.keys(configEnv)) {
        console.log(`[managedEnv] 🔍   ${key} = "${process.env[key]}" (来自 config.yaml)`)
      }
    } else {
      console.log('[managedEnv] 🔍 Step 3: config.yaml 无 env 配置')
    }
  } catch (error) {
    // config.yaml may not exist or fail to load, ignore error
    console.log('[managedEnv] 🔍 Step 3: 加载 config.yaml env 失败:', error)
    logForDebugging('[managedEnv] Failed to load config.yaml env', { level: 'warn' })
  }

  console.log('[managedEnv] 🔍 环境变量应用完成')
