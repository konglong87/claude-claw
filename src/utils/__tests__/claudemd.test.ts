...
import { getUserInfoPath } from "../config.js";
...
describe('UserInfo MemoryType', () => {
  beforeEach(() => {
    // Reset state before each test
    setCurrentUserId(null, null)
  })

  test('getUserInfoPath generates correct path', () => {
    const path = getUserInfoPath('ou_abc123', 'feishu')
    expect(path).toMatch(/\.claude\/soul\/feishu_ou_abc123\.md$/)
  })

  test('getUserInfoPath sanitizes illegal characters', () => {
    const path = getUserInfoPath('user/with:special*chars', 'feishu')
    expect(path).toMatch(/feishu_user_with_special_chars\.md$/)
  })

  test('setCurrentUserId updates state', () => {
    setCurrentUserId('test_user', 'feishu')
    expect(getCurrentUserId()).toBe('test_user')
    expect(getCurrentChannel()).toBe('feishu')
  })

  test('setCurrentUserId with different channels', () => {
    setCurrentUserId('user1', 'feishu')
    expect(getCurrentUserId()).toBe('user1')
    expect(getCurrentChannel()).toBe('feishu')

    setCurrentUserId('user2', 'dingtalk')
    expect(getCurrentUserId()).toBe('user2')
    expect(getCurrentChannel()).toBe('dingtalk')
  })

  test('setCurrentUserId to null clears state', () => {
    setCurrentUserId('user1', 'feishu')
    expect(getCurrentUserId()).toBe('user1')

    setCurrentUserId(null, null)
    expect(getCurrentUserId()).toBe(null)
    expect(getCurrentChannel()).toBe(null)
  })
})
