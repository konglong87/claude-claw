...
export function getUserInfoPath(userId: string, channel: string): string {
  // Sanitize userId to remove illegal path characters
  const safeUserId = userId.replace(/[\/\\:*\?"<>|]/g, '_')
  const projectRoot = getProjectRoot()
  return join(projectRoot, '.claude', 'soul', `${channel}_${safeUserId}.md`)
}
