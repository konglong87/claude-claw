...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process UserInfo file (if userId is set)
    const currentUserId = getCurrentUserId()
    const currentChannel = getCurrentChannel()
    if (currentUserId && currentChannel && isSettingSourceEnabled('projectSettings')) {
      const userInfoPath = getUserInfoPath(currentUserId, currentChannel)
      result.push(
        ...(await processMemoryFile(
          userInfoPath,
          'UserInfo',
          processedPaths,
          false, // UserInfo files don't allow @include external files
        )),
      )
    }
